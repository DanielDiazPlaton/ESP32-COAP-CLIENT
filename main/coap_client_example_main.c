...
#include "driver/gpio.h"  // add to turn gpio
...
static void gpio_on(gpio_num_t pin)
{
    /* Set the GPIO level according to the state (LOW or HIGH)*/
    gpio_set_level(pin, LED_ON);
}

static void gpio_off(gpio_num_t pin)
{
    /* Set the GPIO level according to the state (LOW or HIGH)*/
...
void start_mdns_service(void){
    ...
    // Initialize mDNS services
    mdns_init();
    printf("\nmDNS running....\n");

    // Set hostname
    mdns_hostname_set("esp32_CLIENT");

    // Set instance
    mdns_instance_name_set("ESP CLIENT");
...
void add_mdns_services(void){
    ...
    mdns_txt_item_t serviceTxtData[3] = {
        {"board", "esp32 Client Coap"},
        {"u", "dida"},
        {"p", "12345678"}
    };

    // add services
    mdns_service_add("ESP32 CLIENT COAP", "_http", "_tcp", 80, serviceTxtData, 3);

}

// Function to convert int to char
void enteroACaracter(int numero, char cad[]){
    int numDig = 0;
    int aux = numero;
    while(aux>0){
        aux = aux/10;
                ...
                const coap_mid_t mid)
{
    ...
    if (COAP_RESPONSE_CLASS(rcvd_code) == 2) {
        if (coap_get_data_large(received, &data_len, &data, &offset, &total)) {
            ...
            if( 65 == (int)*data){
                gpio_on(GPIO_NUM_12);
                printf("LED Inside ON\n");
                printf("\n");
            }
            if( 66 == (int)*data){
                gpio_on(GPIO_NUM_25);
                printf("LED Outside ON");
                printf("\n");
            }
            if( 97 == (int)*data){
                gpio_off(GPIO_NUM_12);
                printf("LED Inside OFF");
                printf("\n");
            }
            if( 98 == (int)*data){
                gpio_off(GPIO_NUM_25);
                printf("LED Outside OFF");
                printf("\n");
            }
...
static void coap_example_client(void *p)
{
    ...
    addr.addr = 0;
    esp_err_t err = mdns_query_a("esp32_SERVER", 2000,  &addr);
    ...
    char buffer[200];
    const char * str_uri = "coap://";
    const char * str_uri2 = ":5683/DanielHome";
    str_uri_ip1 = (ip4_addr1(&addr));
    str_uri_ip2 = (ip4_addr2(&addr));
    str_uri_ip3 = (ip4_addr3(&addr));
    str_uri_ip4 = (ip4_addr4(&addr));
    char storge_ip[5];
    enteroACaracter((int)str_uri_ip1, storge_ip);
    strcat(strcpy(buffer, str_uri), storge_ip);
    strcat(buffer, ".");
    enteroACaracter((int)str_uri_ip2, storge_ip);
    strcat(buffer, storge_ip);
    enteroACaracter((int)str_uri_ip3, storge_ip);
    strcat(buffer, ".");
    strcat(buffer, storge_ip);
    enteroACaracter((int)str_uri_ip4, storge_ip);
    strcat(buffer, ".");
    strcat(buffer, storge_ip);
    strcat(buffer, str_uri2);
    // use this printf to degub the uri
    // printf("%s\n", buffer);

    const char       *server_uri = (char *)buffer;//"coap://192.168.137.49:5683/DanielHome";
